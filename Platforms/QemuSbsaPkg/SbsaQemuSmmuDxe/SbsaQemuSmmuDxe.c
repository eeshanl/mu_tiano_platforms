...
{
    SmmuV3PrintErrors(Smmu->SmmuBase);
}

/**
  This routine initializes timer events to poll GError and Event Queue.
**/
VOID
EFIAPI
InitializeTimer (
    ...
    )
{
    ...
    StreamEntry->S2Ttb = (UINT64) PageTableRoot >> 4;
    ...
    )
{
    ...
    Status = SmmuV3BuildStreamTable(SmmuBase, Smmu->StreamTableLog2Size, (VOID *)Smmu->PageTableRoot, &TemplateStreamEntry);
    if (EFI_ERROR(Status)) {
        SmmuV3FreeStreamTable(Smmu->StreamTable, Smmu->StreamTableSize);
    ...
    SmmuV3WriteRegister32(SmmuBase + 0x10000, SMMU_EVENTQ_PROD, 0);
    SmmuV3WriteRegister32(SmmuBase + 0x10000, SMMU_EVENTQ_CONS, 0);
    ...
    )
{
    ...
    DEBUG((DEBUG_INFO, "SmmuConfig->x = 0x%llx\n", SmmuConfig->x));
    ...
    Smmu->SmmuBase = PcdGet64(PcdSmmuBase);
    ...
    Status = SmmuV3Configure(Smmu->SmmuBase, 1);
    if (EFI_ERROR(Status)) {
