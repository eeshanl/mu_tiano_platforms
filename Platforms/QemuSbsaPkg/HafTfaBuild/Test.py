...
def build_haf_tfa():
    ...
    # Define repository URLs and commits
    HAFNIUM_REPO = "https://github.com/TF-Hafnium/hafnium.git"
    HAFNIUM_COMMIT = "9f06689c070af10c1e0f0679942a019207fe2d67"
    
    TFA_REPO = "https://github.com/ARM-software/arm-trusted-firmware.git"
    TFA_COMMIT = "842ce6391fecc83d2371023ffcaceb4c84ac946c"
    
    MU_PLATFORMS_REPO = "https://github.com/microsoft/mu_tiano_platforms.git"
    ...
    if ret != 0:
        ...
        return ret
    # subprocess.run(["git", "config", "--global", "--add", "safe.directory", mu_platforms_path], check=True)
    cmd = "git"
    ...
    with open(sp_layout_file, 'w') as f:
        data = {
            "stmm": {
                "image": {
                    "file": os.path.join(mu_platforms_path, 'Build/QemuSbsaPkg/DEBUG_GCC5', 'FV/BL32_AP_MM.fd'),
                    "offset": "0x2000"
                },
                "pm": {
            ...
            "mssp": {
                "image": {
                    "file": os.path.join(mu_platforms_path, 'Build/QemuSbsaPkg/DEBUG_GCC5', 'FV/BL32_AP_MM_SP1.fd'),
                    "offset": "0x10000"
                },
                "pm": {
    ...
    # # Copy TF-A binaries
    # tfa_build_dir = os.path.join(tfa_path, "build", self.env.GetValue("QEMU_PLATFORM").lower(), 
    #                              "debug" if self.env.GetValue("TARGET").lower() == 'debug' else "release")
